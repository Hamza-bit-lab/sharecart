            ...
            <form action="{{ route('lists.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <div class="d-flex flex-wrap gap-2 mb-2">
                        <input type="hidden" name="icon" id="icon-input" value="{{ old('icon', 'bi-cart4') }}">
                        @foreach(App\Models\GroceryList::RECOMMENDED_ICONS as $icon)
                            <button type="button" class="btn btn-outline-light border emoji-btn p-2 fs-4 {{ old('icon', 'bi-cart4') === $icon ? 'active border-primary bg-light text-primary' : 'text-dark' }}" data-emoji="{{ $icon }}" style="width: 50px; height: 50px;">
                                <i class="bi {{ $icon }}"></i>
                            </button>
                        @endforeach
                    </div>
